# app/ui_components/settings_dialog.py
...
class SettingsDialog(QDialog):
    ...
    def _create_service_tab(self, service_name):
        ...
        btn_add.clicked.connect(lambda _, s=service_name: self.add_key(s))
        btn_edit.clicked.connect(lambda _, s=service_name: self.edit_key(s))
        btn_delete.clicked.connect(lambda _, s=service_name: self.delete_key(s))
        btn_set_active.clicked.connect(lambda _, s=service_name: self.set_active_key(s))
    ...
    def add_key(self, service_name):
        new_key, ok = QInputDialog.getText(self, f"Додати API Ключ для {service_name.capitalize()}", "Введіть новий API ключ:")
        if ok and new_key:
